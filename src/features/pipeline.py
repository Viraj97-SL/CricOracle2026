...
Single entry point for feature engineering. Chains together:
    Raw data → Player profiles → Venue stats → Match context → Team features → Final dataset
...
class FeaturePipeline:
    ...
    def run(self, save: bool = True) -> pd.DataFrame:
        ...
        6. Merge everything into match-level dataset
        ...
        # ============================
        logger.info("--- Step 6: Merging All Features ---")
        ...
        logger.info(f"Target variable 'team1_won': "
    ...
    def _merge_all(
        ...
        # Handle categorical venue column
        ...
        safe_fill = [c for c in numeric_cols if c not in {"team1_won", "first_innings_score", "second_innings_score"}]
